<?php
	...
	if (isset($_GET['promocion'])) {
		...
		if (isset($_GET['zona_id'])) {
			$zona_id = preg_replace('/[^-a-zA-Z0-9_]/', '', $_GET['zona_id']);

			$seleccionar_restaurantes = $conn->prepare("SELECT * FROM restaurantes WHERE zona_id = :zona_id AND promocion != '';");
			$seleccionar_restaurantes->bindParam(":zona_id", $zona_id);
		...
		if (isset($_GET['zona_id'])) {
			...
			$seleccionar_restaurantes = $conn->prepare("SELECT * FROM restaurantes WHERE zona_id = :zona_id");
			$seleccionar_restaurantes->bindParam(":zona_id", $zona_id);
			$seleccionar_restaurantes->execute();
			...
			$seleccionar_zonas = $conn->prepare("SELECT * FROM zonas");
			$seleccionar_zonas->execute();

			$zonas = $seleccionar_zonas->fetchAll(PDO::FETCH_ASSOC);

			$control = 0;

			foreach ($zonas as $zona) {
				$zonas[$control]['zona'] = utf8_encode($zona['zona']);
